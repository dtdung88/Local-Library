...
exports.book_create_get = function(req, res, next) {
    // Get all authors and genres, which we can use adding to our book.
    async.parallel({
        ...
        if (!errors.isEmpty()) {
            ...
            async.parallel({
                ...
                for (let i = 0; i < results.genres.length; i++) {
                    if (_book.genre.indexOf(results.genres[i]._id) > -1) {
                ...
                res.render('book_form', { title: 'Create Book', authors: results.authors, genres: results.genres, book: _book, errors: errors.array() });
            });
            return;
        }
        else {
    ...
    (req, res, next) => {
        ...
        if (!errors.isEmpty()) {
            ...
            async.parallel({
                ...
                res.render('book_form', { title: 'Update Book', authors: results.authors, genres: results.genres, book: _book, errors: errors.array() });
            });
            return;
        }
        else {
            ...
            Book.findByIdAndUpdate(req.params.id, _book, {}, function(err, theBook) {
                if (err) { return next(err); }
